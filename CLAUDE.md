...
- **Root pages**: Main portfolio (`index.html`) and legacy case study page (`case-study.html`)
...
**Primary Design System (`index.html` and newer case studies):**
...
| Portfolio card title/description | `index.html` → project card section |
...
3. **Portfolio card**: `index.html` → project card title/description
...
- Main palette defined in `index.html` (`:root` variables)
...
- Back navigation to `index.html` from case study pages
...
1. **Use the current design system** from `index.html` (DataCamp colors, not pastel)
...
7. **Link back to portfolio**: All case study pages should link to `index.html`
